...
13. 생성한 로드밸런서 서비스의 IP를 확인
14. 호스트 컴퓨터에서 브랑줘를 열고 확인
15. exec를 통해 접속한 파드에서 ls /audit 명령을 실행해 접속 기록 파일이 남았는지 확인
16. 마스터 노드에서 파드를 4개에서 8개로 증가
17. 생성된 파드 확인
18. 최근 증가한 4개의 파드 중 하나를 선택해 접속하고, audit 로그가 동일한지 확인
19. 다른 브라우저를 열어서 다른 파드 이름과 IP가 표시되는지 확인
20. exec로 접속한 파드에서 `ls /audit`을 실행해 시로 추가된 audit 로그 확인
21. 기존 접속한 파드에서도 동리한 로그가 audit에 기록되어있는지 확인